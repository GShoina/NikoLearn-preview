/* ═══════════════════════════════════════════════════════════
   NIKO LEARN: draw.js — „ხატვა" Draw & Color creative canvas (v1.234)
   New play-activity (alongside movement/talk). A real HTML5-canvas paint tool for kids:
   brush · eraser · fill(bucket) · undo · clear · colour palette · brush sizes · colour-in templates · save.
...
const DRAW_COLORS=['#000000','#ffffff','#e23b3b','#f2894c','#e8c45f','#3aa66e','#4d97d1','#6b63b5','#d65aa0','#8a5a3a'];
...
/* ── colour-in templates: simple thick-outline line art the child fills with the bucket/brush ── */
const DRAW_TEMPLATES={
  ...
  house:{ico:'🏠',draw(c,w,h){const s=Math.min(w,h)*0.5,x=w/2-s/2,y=h/2-s/4;
    ...
    c.strokeRect(x+s*0.4,y+s*0.4,s*0.2,s*0.4);                       // door
    c.strokeRect(x+s*0.1,y+s*0.12,s*0.2,s*0.2);}},                   // window
...
function dwStroke(a,b){ _dCtx.strokeStyle=(_dTool==='eraser')?'#ffffff':_dColor; _dCtx.lineWidth=(_dTool==='eraser')?_dSize*2:_dSize;
...
function dwFill(cssX,cssY){
  ...
  const fc=_hexRgb(_dColor); if(!fc)return;
...
function dwPickColor(el,c){ _dColor=c; if(_dTool==='eraser')dwTool(document.getElementById('dwT-brush'),'brush');
...
function dwTool(el,t){ _dTool=t; ['dwT-brush','dwT-fill','dwT-eraser'].forEach(id=>{const b=document.getElementById(id);if(b)b.classList.remove('on');});
