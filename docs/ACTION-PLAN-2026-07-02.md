...
- 2026-07-02: **0.2 ground-truth (კოდი):** ტუტორი ასწავლის პრინციპს ყველა რეჟიმში (`niko/tutor.js` explain/hints თითო mode-ზე); answer-leak fix უკვე ხეშია (`countSeq`/`mulSeq`, tutor.js:14-25). „does nothing" = ძველი ცოცხალი ვერსია, არა კოდი. **რეალური გაპი 0.2-ში:** (ა) `teach_then_retry_result` ივენთი (გაზომვა), (ბ) deploy. NEXT: 0.3 Q1 warm-up (53% q0 abandon) — ხილული, მაღალი-impact.
