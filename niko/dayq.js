/* ═══════════════════════════════════════════════════════════
   ...
   Content = ORIGINAL NikoLearn IP (§11), authored for W7. Bubu VOICES each
   question (auto on open, replay via 🔊) and its answer (on the reveal tap) with
   recorded EkaNeural clips (build-time, tools/_gen_dayq.py) — the sanctioned
   recorded-clip path, NOT runtime ka-TTS (§6). The English reward word stays a
   silent visual pairing (open PI-104 English-voice gate untouched).
   Self-contained: own `dq` state, reuses render()/goHome()/playClip(). Chrome bilingual.
...
const DAYQ = [
  ...
  {ic:'😋', col:'#C96F4A', sv:'yummy', tag:'სხეული', q:'რატომ მოგვდის ნერწყვი, როცა გემრიელ საჭმელს ვხედავთ?', a:'შენი პირი საჭმელს წინასწარ ემზადება! ნერწყვი საჭმლის დაღეჭვასა და გადაყლაპვაში გვეხმარება. თვალები ხედავენ, ტვინი კი პირს ეუბნება: მოემზადე, გემრიელი მოდის!', en:'yummy = გემრიელი'},
  {ic:'🍂', col:'#B0762E', sv:'tree', tag:'ბუნება', q:'რატომ სცვივა ხეს ფოთლები შემოდგომაზე?', a:'ზამთარში ხისთვის ფოთლების გამოკვება ძალიან ძვირია: წყალი იყინება და საჭმელი ცოტაა. ამიტომ ხე ფოთლებს „შვებულებაში უშვებს" და თვითონ იძინებს. გაზაფხულზე ახალ ფოთლებს გამოიღებს!', en:'tree = ხე'},
...
function dayqReveal(){
  ...
  // Bubu speaks the answer (recorded ka clip); stop the question clip first so they never overlap
  try{ if(typeof stopAudio==='function') stopAudio(); }catch(e){}
  try{ if(dq && typeof playClip==='function') playClip(DAYQ[dq.i].a); }catch(e){}
}

/* play the current question clip (recorded EkaNeural); silent no-op if the clip is missing */
function dayqSay(){
  if(!dq) return;
  try{ if(typeof stopAudio==='function') stopAudio(); }catch(e){}
  try{ if(typeof playClip==='function') playClip(DAYQ[dq.i].q); }catch(e){}
}
...
function renderDayQ(){
  ...
  render(`<div class="screen dayq">
    ...
    <div class="dayq-stage" style="background:linear-gradient(165deg,${t[0]},${t[1]})">
      <div class="qcard" style="border-color:${t[2]}66;box-shadow:0 2px 6px ${t[2]}22,0 16px 34px ${t[2]}2e">
        ...
        <h3 class="dayq-q">${c.q} <button class="dayq-say" onclick="dayqSay()" aria-label="${en?'Hear the question':'მოისმინე კითხვა'}" style="display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;vertical-align:middle;border:0;border-radius:50%;background:${t[2]}1f;color:${t[2]};font-size:.9rem;cursor:pointer;margin-inline-start:6px">🔊</button></h3>
  ...
  dayqSay(); // Bubu reads the question aloud on open / on each nav (recorded ka clip)
}
