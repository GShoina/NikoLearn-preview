...
| W4 | Weekly parent card — PROTOTYPE + show | Feature 4 of the accepted list. | **SHOWN 2026-07-20** — same file as W1; all-local stats, feeds W2 card; W5 strengthens its letter data · **UPDATE 2026-07-22: MORE acceptable (owner's most-approved of the batch) — one required change before ship: apply the text-readability standard so a parent grasps it in human Georgian, main points as BULLETS (no dense prose). Then ship.** · **DONE 2026-07-22 → readable v2 built:** `output/2026-07-22-W4 კვირის ბარათი წაკითხვადი ვერსია by Niko.html` (§6c-verified render). BLUF one-liner + 4 scannable bullets + concrete „this-week's-work" + warm owl line; aligned to `READABILITY_STANDARD.md`; old v1 kept side-by-side (ჩანაცვლება≠წაშლა). Awaits owner nod, then app-integration in `parent.js`. |
