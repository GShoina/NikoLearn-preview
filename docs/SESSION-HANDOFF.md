...
> **🔴 OWNER DECISION 2026-06-27 — CONTENT IP (locked, CLAUDE.md §11):** the Kings content was found transcribed/translated from real kings.ge PDFs item-for-item (file header literally says "authored from real kings.ge PDFs"; comments say "faithful to PDF"). Owner ordered: NEVER copy as-is, ALWAYS create ORIGINAL NikoLearn IP (be the "creative teacher"), inspired only by skill/format/difficulty. Standing rule for ALL content. **IN PROGRESS: full rework of Kings question content to original-only**, keeping the (non-copyrightable) section format. v1.266 stays live during rework (low risk; can roll Kings tiles offline in ~2 min if owner wants zero exposure). g4-6 first (kings-content.js), then check/rework g2-3 (KEX_*/KMX_* in kings-exam.js).
