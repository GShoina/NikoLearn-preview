...
**Updated: 2026-06-04 ~17:40 | Resume pointer: live at v2.6. Math content track "A" IN PROGRESS — batch 1 DONE & live (comparison >/<, skip-counting 5s/10s, shapes; new game modes in games.js: cmpRound/skipRound/shapeRound, SHAPES data in data.js, math-menu tiles in screens.js, owl hints in tutor.js, i18n labels, parent-dashboard labels). Shape option text renders per-UI-language (never collides with vocab content). NEXT (A batch 2): money (₾/თეთრი) + clock/time. Also done earlier today: v2.5 math gentle progression (1-20→1-40→1-70→1-100, 2-strong-rounds to advance) fixing Niko's "tests go above 20". Earlier: v2.4
