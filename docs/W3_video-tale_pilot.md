...
> Status: **SCRIPT drafted + loop-წესი self-passed. PENDING Gemini gate (step 2, MANDATORY).**
...
5. **DISCOVERY 3** (~7s): „დღისით კი მზე ძალიან კაშკაშაა ჩემი თვალებისთვის. ამიტომ ვისვენებ."
6. **REVEAL** (~8s): ანუ ბუბუ ღამის მეგობარია. ის მუშაობს, როცა ჩვენ გვძინავს, და ისვენებს, როცა ჩვენ ვთამაშობთ.
...
| 4 | Natural Georgian, no em dash | 8* | *self-score only — NOT final. Gemini gate + owner validate (§6f). |
