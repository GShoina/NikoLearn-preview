...
> **STILL PENDING (owner's multi-step program, in order):** (2) whole-app agent-team deep debug [NEXT]; (3) Georgian copy report incl. the deferred Kings G2-G7 consistency items (review-only, owner+Gemini); (4) curriculum next-step (recommend→plan→build).
