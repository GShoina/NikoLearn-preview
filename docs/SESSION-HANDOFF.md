...
**Updated: 2026-06-04 ~16:30 | Resume pointer: live at v2.2 — English UI (ka/EN toggle) + web fonts restored + 4-palette color-theme switcher on the landing (sunlit/ocean/forest/berry). On mobile the language toggle + theme switcher live INSIDE the hamburger menu (bar = wordmark + CTA + hamburger); desktop keeps them in the nav bar. Designer's `Downloads/NikoLearn Landing PRODUCTION.html` was the source for fonts+theme; backup at `backups/landing_pre-theme_2026-06-04.html`. GA4 live-metrics proxy is still CODED but NOT deployed — blocked by Google (clasp's OAuth app is hard-blocked for sensitive scopes; would need a self-owned GCP OAuth client). Owner decided GA4 live tiles not worth it for now (admin already deep-links to GA4 + Leads). On resume: take a new request, or finish GA4 only if owner wants the GCP OAuth-client route.**
