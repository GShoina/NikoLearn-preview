...
- 2026-07-03: **0.1 გაძლიერება — ვიზუალური გეითი ჩაერთო pre-push-ში.** `.git/hooks/pre-push` (main-ref) ახლა `npm test`-ის შემდეგ `npm run qa:visual`-საც უშვებს: რეალური ლეიაუტ-ოფენდერი (exit 1) აბლოკებს push-ს, chromium-ის უქონლობა (exit≠0/1) = graceful SKIP (legit deploy არ იბლოკება). end-to-end დატესტილი (main→ორივე გეითი pass→exit 0; non-main→skip). ⚠ hook `.git/hooks/`-შია = **არა version-tracked** (re-clone-ზე უნდა აღდგეს); არსებული hook-კონვენცია იგივეა.
