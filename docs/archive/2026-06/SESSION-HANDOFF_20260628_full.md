...
>
> ## ▶ RESUME NOW (2026-06-28 · evening) — **bug-fix + infra sprint shipped (v1.284→v1.289); centre-line writing PARKED on a branch.**
> **LIVE on nikolearn.com now: v1.289.** Session deploys (all on GShoina/NikoLearn main, live-verified):
> - **v1.284** parent-reported fixes: Georgian dative grammar „დათვი/ბავშვი სძინავს" → „დათვს/ბავშვს სძინავს" (data.js + regenerated edge-tts clips krd_040/sent_11 + audio-manifest keys); cross-screen **audio bleed** fixed (render() calls `window.stopAudio()`); **U3** 🔊 voice-toggle label follows interface lang (`vtglLabel` in core.js).
> - **v1.285** #5 hide voice-toggle in alphabet sections (ka-alpha/en-alpha); #7 colourful counting (`.count-card` + per-tile pastel `.opt.cnum` in games.js `nextCount`).
> - **v1.286** telemetry **batching** (analytics.js queues events, flush debounce 5s + on pagehide) → fixes the Cloudflare **KV 429 daily-write** cap that was dropping data on 400+ event days. (The 2026-06-14 rate-limit was abuse-only, NOT this.)
> - **v1.287** privacy.html **microphone** section (ka+en) + landing „GDPR & KIDS" badge → „GDPR-ის პრინციპებზე აგებული".
> - **v1.288** #8 count-along (`countAlong` in games.js: lights each object 1..N + speaks the number).
> - **v1.289** #9 trace animation contour-by-contour (`watchStrokes`) + **Kings landing fix** (un-hid on mobile + „ეფუძნება კინგსის მეთოდიკას"→„კინგსისთვის მზადება", landing.html).
> **WORKER (nikolearn-t.bivision.workers.dev) REDEPLOYED:** CORS `ALLOWED_ORIGINS` now includes nikolearn.com + www (was gshoina.github.io only) → **parent feedback now actually delivers** (was 0 rows despite feedback_open). Deployed via wrangler + CF_NIKO_API_TOKEN. Verified `{"ok":true}` from nikolearn.com origin. Test row cleaned from KV.
> **www DNS:** added CNAME `www → gshoina.github.io` at domenebi.ge (Premium DNS = AWS Route53; via playwright, owner already logged in). http://www → 301 → nikolearn.com works. **www HTTPS BROKEN:** GitHub Pages cert covers ONLY apex (cert domains=["nikolearn.com"]); www not in SAN → `ERR_TLS_CERT_ALTNAME_INVALID`. Re-trigger via `gh api PUT pages cname=nikolearn.com` did NOT add www. **FIX (when wanted, small live risk): remove+re-add the GitHub Pages custom domain so the cert re-issues with the www CNAME present, OR set custom domain to www.** Meanwhile: USE BARE `https://nikolearn.com` (works); use bare in all FB ads.
> **CENTRE-LINE one-stroke handwriting (parent #9 deep work) = PARKED, owner paused.** Branch **`feat/centerline-writing`** (commit 8e75aec). Algorithmic: real font ka.ttf → rasterize → Zhang-Suen thinning → trace → smooth → `niko/stroke-data.js` (33 letters). `scratchpad/skel.cjs` = the generator; `scratchpad/grid.html` = visual grid. Integrated into alpha.js `traceLearn`/`animateCenterline` + index.html + sw.js (ALL on the branch, NOT main). Verified: AVG recall 0.99, upright, smooth, recognisable. **Remaining before it can ship:** (a) stroke ORDER/DIRECTION per the standard (currently algorithmic — some letters start from a non-ideal end); (b) fix weak letters ი, ო (hole-split). **Lesson:** numeric self-test couldn't catch the upside-down bug (compared to its own raster) — always visually compare to an upright reference.
> **STILL OPEN (owner forks / not done):** #11 fonts self-host (Google-Fonts IP-leak, privacy/FB-ad); #12 in-app signal (fake-door „Premium" intent + feedback nudge) = owner's #1 priority but customer-facing; #13 Meta Pixel decision · „Kings" legal-affiliation · owl-logo origin; Kings positioning (parallel strategy). Viktor pre-FB-ad audit confirmed live: www#2 + CORS#6 fixed; mic#3 + badge#5 shipped; Pixel#1 + Kings#4 = owner.
>
> ## ▶ RESUME NOW (2026-06-28 · latest) — **English voice + on-device metrics + design brief v2 shipped.**
> **Version now: v1.283, on `main` (GShoina/NikoLearn), all live-verified.** Session commits: `30530ad` (EN voice v1.282), `a8461cc` (metrics + docs v1.283), `10dec23` (design brief v2). Backup before EN audio: `backups/2026-06-28_pre-en-audio/`.
> **✅ ENGLISH VOICE — shipped live v1.282 (`30530ad`).** English was on fragile/silent runtime browser-TTS. Now: 926 genuinely-spoken English strings (vocab, alphabet, shapes, Kings options/answers, YLE listening + story sentences, praise) pre-rendered as **edge-tts en-GB-Sonia rate=-10%** clips (`niko/audio/en_NNNN.mp3`) wired into `AUDIO_MANIFEST` by exact lowercased text key. Fill-in-the-blank stems deliberately EXCLUDED (read visually; TTS of `___` garbles) → keep TTS fallback. Self-tested in real browser (every EN word → its clip; Georgian unaffected; 0 errors), live-verified on nikolearn.com. Extractor + generator: `scratchpad/extract_en.cjs`, `gen_en.cjs`, `inject_manifest.cjs`. **VOICE DECISION [USER]: Sonia −10% = MVP voice, $0, reversible placeholder (NOT a brand moat — generic TTS).** Owner picked it after hearing 4 voices + tempo samples (`output/en-sample/`).
> **✅ ON-DEVICE DIFFICULTY METRICS — shipped live v1.283 (`a8461cc`).** Closes the "we improve what we can't measure" gap. New `niko/metrics.js`: measures **first-attempt success per mode** (target band 70-85%) + replay + time, **100% on-device (localStorage `niko_metrics`), no backend, no PII, COPPA-safe.** Formula: `(shields − missMap.size)/shields`. Hooks: `results()` (games.js) + `kxFinish()`+`kx.wrong` counter (kings-exam.js). **Owner-only readout** in the parent dashboard, gated on `niko_owner==='1'` (hidden from real parents). Loaded after core.js; precached in sw.js. Self-tested in browser (bands compute right, 0 errors). This is what lets the owner's family voice-testing actually SHOW if items are in-band.
> **✅ DATA HYGIENE — `docs/DATA_HYGIENE.md` (SSOT).** Exclude owner/tester by **DEVICE, never IP** (we never store IP — privacy/COPPA). Mechanism: open `nikolearn.com/?notrack=1` once per device → sticky `niko_owner=1` → excluded forever, any network. Home IP `178.134.159.212` recorded REFERENCE-ONLY (not used for filtering). **OWNER ACTION: open `?notrack=1` on each test device (phone, laptop) so family testing doesn't pollute stats.**
> **✅ DESIGN BRIEF v2 — `docs/DESIGN_BRIEF_PROMPT_v2.md` (canonical, send THIS to the designer).** Owner critiqued v1 as too interface-focused, not enough product/mission. v2 restructured: **MISSION TEST (curiosity/confidence/creativity) = supreme criterion at top**, emotional layer leads BEFORE the tech contract (discovery-not-completion, one-emotional-question-per-screen, confidence-over-beauty, tutor personality/Amonashvili, reward philosophy, parent trust). Reconciled discovery↔Guided-Journey; tutor "asks questions" scoped to scripted (no live-AI chat). **OLD `docs/DESIGN_BRIEF_PROMPT.md` = DELETE — currently LOCKED (open on owner's machine); owner closes it → agent deletes.**
> **GEORGIAN VOICE: prep only, no change this session.** Existing 467 edge-tts `ka-GE-EkaNeural` clips unchanged. Ready artifacts: demo package `output/2026-06-28-ქართული ხმის დემო-ჩაწერა by Niko.html` (33 phrases for a human) + Owl Voice Pack ka list `output/2026-06-28-Owl Voice Pack phrase list by Niko.html`. **Owner plan: family records voices → agent drops into `niko/audio/` by exact filename (drop-in, code unchanged) → test.**
> **PRO-ACTOR VOICE PACK = ROADMAP, no spend.** [USER] no money until monetization/sales start. ~350-phrase recorded-actor pack (ka+en) is the real brand moat, drafted, parked. Reserved budget $10-20 for "fix existing voice IF needed" UNSPENT — agent recommendation: keep the $0 edge-tts floor, don't spend (marginal, non-moat).
> **OPEN / NEXT (this session):** (1) **parent-reported bugs INCOMING** — owner will send concrete bugs the parent wrote → fix any not already fixed, autonomously. (2) **Challenge-checklist** for scoring the designer's returned work against brief v2 — offered, not yet built. (3) delete old design-brief file once unlocked. (4) family Georgian recording when owner ready.
> **PROCESS SELF-CORRECTION (owner-flagged 2026-06-28):** owner-facing replies default to GEORGIAN (I drifted to English in technical/execution turns — real lapse). English only for artifacts (designer prompt, code, filenames).
>
> ## ▶ RESUME NOW (2026-06-27) — **CRASH RECOVERY. Kings exam grades 4-6 WIP recovered from disk, checkpointed, runtime-verified.**
...
> **✅ SHIPPED LIVE v1.265 (2026-06-27):** owner chose to REMOVE the BONUS6-Q4 bank item (done). Also applied: RC6-5 grammar reword, harbour. Merged feat/kings-g4-6 → main (`af74ee8..d9b062e`), pushed to GShoina/NikoLearn, Pages rebuilt. Live-verified (SW cleared + reload): APP_VERSION 1.265, KINGS_EXAM grades 2-6 both subjects, C-1 escaper present, KMX_BONUS6=7 items, 0 console errors. Rollback anchor: v1.264=af74ee8, v1.265=d9b062e.
> **🔴 OWNER DECISION 2026-06-27 — CONTENT IP (locked, CLAUDE.md §11):** the Kings content was found transcribed/translated from real kings.ge PDFs item-for-item (file header literally says "authored from real kings.ge PDFs"; comments say "faithful to PDF"). Owner ordered: NEVER copy as-is, ALWAYS create ORIGINAL NikoLearn IP (be the "creative teacher"), inspired only by skill/format/difficulty. Standing rule for ALL content. **✅ DONE for g4-6 — shipped live v1.267 (`1619e2c`).** Full rework of Kings question content to ORIGINAL NikoLearn IP: 6 creative-teacher author agents (one per grade/subject) replaced all g4-6 pools with brand-new questions (public-domain archetypes + own numbers/scenarios, NO brand names, no PDF items), keeping the non-copyrightable section format. **259 items**; structural validator PASS (every answer∈opts, no em-dash, no brand names, no `"` in g6); 2 independent adversarial verifiers re-derived correctness: math 121/121 correct, English 137/138 (fixed 1 ambiguous a/an item the→of). Run-the-app: g6-math exam plays with new content, advances, 0 console errors. Live-verified: v1.267, honeybee reading set present, magti/brand=0. Validator script: scratchpad/validate_kings.js. **✅ g2-3 ALSO DONE — shipped live v1.268 (`c4cb55a`).** grades 2/3 in kings-exam.js were confirmed PDF-derived (header said "Built from REAL kings.ge PDFs"; included literal "LIBERTY" Liberty-Bank task + "ბიუ-ბიუ" brand). 2 author agents replaced all g2-3 English+Math pools with original content; LIBERTY bonus generator → "NIKO" (NikoLearn's own); ბიუ-ბიუ + Magti brand mentions scrubbed (incl. comments). Validator: 440 pooled items across grades 2-6, 0 problems, no brand/copied terms, no em-dash. Run-the-app: NIKO bonus works, g2 exam plays, 0 errors. Live-verified: v1.268, NIKO present, LIBERTY:0, ბიუ-ბიუ:0. **ALL Kings content (g2-6) is now original NikoLearn IP.** Validators: scratchpad/validate_kings.js (g4-6) + validate_all.js (all grades). Rollback anchors: v1.267=1619e2c, v1.268=c4cb55a.
> **✅ UNIFIED 3-LEVEL MODEL — shipped live v1.269 (`cdfca19`).** Owner challenge (2026-06-27): Kings English had a 3-LEVEL selector (Starters/Movers/Flyers = დამწყები/საშუალო/მაღალი) but (1) Math had no level bar, (2) a conflicting GRADE chooser (2-6) was added on top, (3) policy drifting case-by-case. FIXED into ONE axis: 3 levels drive BOTH subjects, BOTH exam + practice. Changes: `kxGrade`→`levelToGrade()` (1→g2, 2→g4, 3→g6); `kxPick()` now runs the exam at current level (grade chooser screen removed); `kingsLevelBar()` made subject-aware + added to kings-math menu; `setKLevel(n,subj)`; math practice tiers (`patternRound`/`reasonRound`) now read `kingsLevel()` not per-mode mastery tier; subtitle „მე-2-6 კლასი"→„შენი დონე" (KA+EN i18n). **Big-picture call (owner-flagged, owner OK'd „go"):** did NOT fold g3/g5 into g4/g6 (g3 easier than g4 → would dilute level-2 difficulty + reintroduce inconsistency); g3/g5 parked in-code, recoverable. Verified runtime (branch): L1/L2/L3 exam → grade 2/4/6 distinct content; math level bar present; 0 errors. Live-verified v1.269 (curl): levelToGrade present, grade-chooser gone, math-practice reads kingsLevel. NOTE: GitHub-Pages HTTP cache serves stale sub-scripts in the DEV browser (not just SW) — `?v=` busts only index.html; verify deployed code via cache-busted curl on raw files. Preview-repo push (GShoina/NikoLearn-preview) is BLOCKED by the safety classifier (external repo force-push = flagged); use screenshots channel instead, or owner runs the preview push. Rollback: v1.268=c4cb55a, v1.269=cdfca19.
> **✅ WHOLE-APP DEEP DEBUG (#2) — shipped fixes v1.270 (`dd2ddfa`).** 5-agent team (2 code, security, UI, UX) per AGENT_TEAM_STANDARD, RUN-THE-APP, 0 console errors in real flows. Found 3 HIGH/8 MED/~12 LOW. FIXED+LIVE: CORE-1 (my 3-level regression — timed practice-exam was locked forever for level≥2; now unlocks at level≥2 + difficulty follows level), U2 (results showed „მე-6 კლასი" → now level name), S1 (stored XSS via restore backup-code → sanitize names), U1 (owner/admin panel reachable by any parent → gated on niko_owner). **OWNER-DECISION items still OPEN (in report output/2026-06-27-მთლიანი აპის deep debug by Niko.html):** S2 Google-Fonts privacy-leak · S3/S4 worker auth+CORS (S4: feedback may fail from nikolearn.com → verify) · S5/U6 weak parent gate + external links · U3 voice-badge stays KA in EN · U4 36-Q exam length · U5 dashboard order. LOW list in report.
> **✅ GEORGIAN COPY REPORT (#3) — delivered, AWAITING OWNER MARKUP** (output/2026-06-27-ქართულის copy report by Niko.html). 23 items, §8 em-dash violations=0. Dominant = terminology inconsistency (ააწყვე/შეადგინე, ვარიანტი/პასუხი, ტესტი/გამოცდა, დაწყებითი/დამწყები, მსწავლელი/მოსწავლე...). Owner marks ✓/✗ → then I APPLY (review-only, not yet changed). I will separately fix the 3 stale em-dash i18n regex matchers (functional EN-translation gap, not copy).
> **✅ CURRICULUM ADD (#4) — Triangle puzzle shipped live v1.271 (`aaff44d`).** 🔺 სამკუთხედი strand in kings-math: (top+bottomLeft)×bottomRight=centre. New `genTriangle(tier)` + REASON_STRANDS entry + tile; reuses reasonRound engine so it auto-follows kingsLevel (level-aware). 300/300 answers verified, visual gate passed (screenshot). Alternative parked: English Listening (Cambridge YLE) = higher-ceiling next option.
> **OWNER FEEDBACK (locked this session):** STOP asking „go" to deploy clear BUG FIXES — bugs are unambiguously to-fix, never gated. Present-first ONLY for: new design/content (taste) + explicit review-deliverables (e.g. Georgian markup). See [[feedback_nikoland_no_yesno_questions]].
> **✅ SOLVABILITY/WELL-POSEDNESS AUDIT + FIXES (2026-06-27, owner-triggered).** Owner caught the Triangle puzzle was ABSURD: showed 3 corner numbers + "?" with the rule `(top+bL)×bR` NEVER shown → unsolvable; my earlier QA verified answer-CORRECTNESS not SOLVABILITY (real blind spot). Ran a dedicated solvability audit (2 agents, every generator + 440 content items). FOUND + FIXED (all live): Triangle REDESIGNED (v1.275 — now shows 3 SOLVED example triangles so the rule is inferable + a uniqueness guard; 3000-run = 0 ambiguous, 0 wrong); g6 READING said "წაიკითხე ტექსტი" with no passage → honest instr + pollen/smoke distractor; 2 g5 finance items silently needed month-length → now state it; "ხუთი თითი"(finger/toe)→"ხელის ხუთი თითი"; "მაღალი ხე" high→wide distractor (v1.274). **STANDING FIX:** WELL-POSEDNESS/SOLVABILITY gate added to AGENT_TEAM_STANDARD.md §4 (answer-correct ≠ well-posed). STILL OPEN: 🧩 genPattern tier-3 interleaved (technically solvable, ~impossible for the age — fixing now). Today reached v1.275. Validators in scratchpad.
> **Label fix shipped v1.266 (live, curl-verified):** Kings tile subtitle was stale "მე-2/3 კლასი" → now "მე-2-6 კლასი" (+EN "grades 2-6"); that's why the new grades looked "not added" — the entry point hid them. Exam is NOT premium-locked (isFreeMode=true).
> **STILL PENDING (owner's multi-step program):** (A) Kings content original-rework [ACTIVE]; (2) whole-app agent-team deep debug; (3) Georgian copy report incl. deferred Kings G2-G7 (review-only, owner+Gemini); (4) curriculum next-step (recommend→plan→build).
