...
(function(){
  ...
  function math(q,kid){
    ...
    if(op==='add'){
      ...
      const h2 = (!kid&&need>0&&need<S)
        ...
        : (kid?`დაითვალე თითებზე 🖐️: ${B}-ს დაუმატე კიდევ ${S}.`:`დაშალე ათეულებად და ერთეულებად: ათეული = ათ-ათი (10, 20, 30), ერთეული = თითო (1, 2, 3). ჯერ ათეულები შეკრიბე, მერე ერთეულები.`);
  ...
  window.Tutor = {
    build(ctx){
      ...
      if(ctx.mode==='yesno'){
        r={hints:['დააკვირდი სურათს. წინადადება ემთხვევა იმას, რასაც ხედავ?','თუ ემთხვევა → ✅ კი. თუ არა → ❌ არა.'],explain:'წაიკითხე წინადადება, შეადარე სურათს. სიმართლეა → კი, არა → არა.'};
        r.name = ctx.aiRole==='coach' ? 'ბუ · ინგლისურის მასწავლებელი' : 'ბუ · შენი მეგობარი';
        return r;
      }
      if(ctx.mode==='speak'){
        r={hints:['ჯერ მოუსმინე კითხვას 🔊, მერე ხმამაღლა უპასუხე ინგლისურად.','სრული წინადადებით თქვი. მაგ. „It is a dog. It is brown."'],explain:'ლაპარაკი ვარჯიშია: ხმამაღლა თქვი პასუხი ინგლისურად. სწორი თუ არასწორი არ ფასდება, მთავარია ცადო.'};
        r.name = ctx.aiRole==='coach' ? 'ბუ · ინგლისურის მასწავლებელი' : 'ბუ · შენი მეგობარი';
        return r;
      }
      if(ctx.mode==='story'){
        r={hints:['ხელახლა წაიკითხე ამბავი ნელა, ან მოუსმინე 🔊.','იპოვე ის წინადადება, სადაც პასუხია. კითხვის მთავარ სიტყვას მიჰყევი.'],explain:'ჯერ ამბავი წაიკითხე, მერე კითხვა. პასუხი თვითონ ტექსტშია დამალული.'};
        r.name = ctx.aiRole==='coach' ? 'ბუ · ინგლისურის მასწავლებელი' : 'ბუ · შენი მეგობარი';
        return r;
      }
      switch(ctx.subject){
